...
export const getSubNotesByNote = asyncHandler(async (req, res) => {
    ...
    return new ApiRespose("SubNotes fetched successfully", 200, subNotes).send(res)
})
...
export const getSubNote = asyncHandler(async (req, res) => {
    ...
    const subNote = await SubNote.findOne({ _id: req.params.subNoteId, note: req.params.noteId })
    if (!subNote) throw new ApiError("SubNote not found", 404)

    return new ApiRespose("SubNote fetched successfully", 200, subNote).send(res)
})
...
export const createSubNote = asyncHandler(async (req, res) => {
    ...
    return new ApiRespose("SubNote created successfully", 201, subNote).send(res)
})
...
export const updateSubNote = asyncHandler(async (req, res) => {
    ...
    return new ApiRespose("SubNote updated successfully", 200, subNote).send(res)
})
